...
function open(cell)
    if cell.s==1 then return end -- do nothing if flag
    if cell.s==0 then cell.s=2 end -- open cell
    if cell.v==0 then
      ...
      if check_win(cells) then win() end
...
function check_win(cells)
  for _,cell in pairs(cells) do
    -- if cell closed or flagged and no mine then no win
...
function TIC()
  if game==0 then tick=tick+1 end
  input()
  draw()
end
